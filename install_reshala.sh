...
# ============================================================ #
# ==      ИНСТРУМЕНТ «РЕШАЛА» v0.295 dev - ДИНАМИЧЕСКИЙ ИНТЕЛЛЕКТ ==
# ============================================================ #
# ==    Добавлено автоопределение сервисов и умное меню.      ==
# ============================================================ #

set -euo pipefail

# --- КОНСТАНТЫ И ПЕРЕМЕННЫЕ ---
readonly VERSION="v0.295 dev"
...
check_for_updates() {
    ...
    if [[ "$LATEST_VERSION" != "$VERSION" ]]; then
        ...
        if [[ "$HIGHEST_VERSION" == "$LATEST_VERSION" ]]; then UPDATE_AVAILABLE=1; fi
    fi
}

run_update() {
    read -p "   Доступна версия $LATEST_VERSION. Обновляемся? (y/n): " confirm_update
    if [[ "$confirm_update" != "y" && "$confirm_update" != "Y" ]]; then echo -e "${C_YELLOW}🤷‍♂️ Твоё дело.${C_RESET}"; wait_for_enter; return; fi
...
detect_services() {
    ...
    if [ -n "$bot_compose_path" ]; then
        ...
        # Ищем версию в файле VERSION, если он есть
        if [ -f "$BOT_PATH/VERSION" ]; then
            BOT_VERSION=$(cat "$BOT_PATH/VERSION")
        else
            # Если нет, берем из тега образа
...
apply_bbr() {
    ...
    if [[ ("$current_cc" == "bbr" || "$current_cc" == "bbr2") && "$current_qdisc" == "cake" ]]; then
        echo -e "${C_GREEN}✅ Ты уже на максималках. Не мешай машине.${C_RESET}"; log "Проверка «Форсаж»: Максимум."; return
    fi

    if [[ ("$current_cc" == "bbr" || "$current_cc" == "bbr2") && "$current_qdisc" == "fq" && "$cake_available" == "true" ]]; then
        echo -e "${C_YELLOW}⚠️ У тебя неплохо (BBR+FQ), но можно лучше.${C_RESET}"
        read -p "   Хочешь апгрейд до CAKE? (y/n): " upgrade_confirm
        if [[ "$upgrade_confirm" != "y" && "$upgrade_confirm" != "Y" ]]; then echo "Как скажешь."; return; fi
        echo "Красава. Делаем как надо."
    elif [[ "$current_cc" != "bbr" && "$current_cc" != "bbr2" ]]; then
        echo "Ездишь на стоке. Включаем форсаж."
    fi
    ...
    local tcp_fastopen_val=3
    local CONFIG_SYSCTL="/etc/sysctl.d/99-reshala-boost.conf"
    log "🧹 Чищу старое..."; sudo rm -f /etc/sysctl.d/*bbr*.conf /etc/sysctl.d/*network-optimizations*.conf
...
net.ipv4.tcp_fastopen = $tcp_fastopen_val" | sudo tee "$CONFIG_SYSCTL" > /dev/null
    log "🔥 Применяю..."; sudo sysctl -p "$CONFIG_SYSCTL" >/dev/null
    echo ""; echo "--- КОНТРОЛЬ ---"; echo "Новый алгоритм: $(sysctl -n net.ipv4.tcp_congestion_control)"; echo "Новый планировщик: $(sysctl -n net.core.default_qdisc)"; echo "----------------"
    echo -e "${C_GREEN}✅ Твоя тачка теперь — ракета. (CC: $preferred_cc, QDisc: $preferred_qdisc)${C_RESET}";
}

# --- IPv6 МОДУЛЬ ---
check_ipv6_status() {
    if [ ! -d "/proc/sys/net/ipv6" ]; then echo -e "Статус IPv6: ${C_RED}ВЫРЕЗАН ПРОВАЙДЕРОМ${C_RESET}";
    elif [ "$(cat /proc/sys/net/ipv6/conf/all/disable_ipv6)" -eq 1 ]; then echo -e "Статус IPv6: ${C_RED}КАСТРИРОВАН${C_RESET}";
    else echo -e "Статус IPv6: ${C_GREEN}ВКЛЮЧЁН${C_RESET}"; fi
}

disable_ipv6() {
    if [ ! -d "/proc/sys/net/ipv6" ]; then echo -e "❌ ${C_YELLOW}Нечего отключать. Провайдер уже всё отрезал.${C_RESET}"; return; fi
...
EOL
    ...
    log "-> IPv6 кастрирован."; echo -e "${C_GREEN}✅ Готово.${C_RESET}"
}

enable_ipv6() {
    if [ ! -d "/proc/sys/net/ipv6" ]; then echo -e "❌ ${C_YELLOW}Нечего включать. Отрезано с корнем.${C_RESET}"; return; fi
    if [ ! -f /etc/sysctl.d/98-reshala-disable-ipv6.conf ] && [ "$(cat /proc/sys/net/ipv6/conf/all/disable_ipv6)" -eq 0 ]; then echo "✅ IPv6 и так работает."; return; fi
    echo "💉 Реанимация IPv6..."
...
EOL
    ...
    log "-> IPv6 реанимирован."; echo -e "${C_GREEN}✅ Готово.${C_RESET}"
}
